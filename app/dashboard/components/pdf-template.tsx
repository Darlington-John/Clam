...
const PDFTemplate = (props: any) => {
   ...
   return (
      <div
         className="flex w-[768px]   p-8 gap-8 flex-col  bg-lightestGrey"
         id="to-image"
      >
         <div className="flex items-center justify-between">
            <Image src={logo} alt="" width={100} height={100} />
            <h1 className="text-[22px] text-black fancy">Statement</h1>
         </div>
         <div className="flex flex-row   items-center  flex-wrap  gap-x-2   gap-y-4  p-4  justify-between">
            {details.map((data, index: any) => (
               <div
                  ...
                  key={index + 1}
               >
                  <h1 className="text-grey  text-[9px] norm-mid uppercase">
                     {data.header}
                  </h1>
                  <h1 className="text-[17px] fancy ">{data.body}</h1>
               </div>
            ))}
         </div>
         <div className="flex flex-col  gap-4 w-full">
            <h1 className="text-[17px] fancy">Transactions</h1>
            <div className="flex flex-col  w-full ">
               <div className="flex items-center w-full  pb-4   bg-lightGrey border  border-lightGreyBorder rounded-t-lg ">
               ...
               {bookData?.entries.map((entry: any, index: number) => (
                  <div
                     className={`flex items-center w-full  min-h-[40px] bg-white  border-b border-x  border-[#DFDDE3] opacity-100 duration-300   ${
                  ...
                  >
                     ...
                     <div className="w-[20%] h-full text-start  flex items-center  px-3  ">
                        <h1 className="text-[9px] py-1 px-2  p rounded  border border-lightGreyBorder  pb-4">
                           {entry.tag || '-'}
                        </h1>
                     </div>
