...
const Login = () => {
    ...
    return (  
        ...
        <h1 className="fancy text-[43px] text-center lg:text-3xl">Welcome back</h1>
        <Link href={'/auth/sign-up'} className="underline text-purple norm-mid  text-[17px] text-center md:text-sm">
       Create account instead?
        </Link>
        <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-1 ">
            <div className="relative w-full flex items-center justify-center">
        <input className={`h-[40px] py-1 px-3 bg-lightGrey text-black  text-sm rounded-lg border  focus:ring-2 focus:bg-lightPurple   ring-purple outline-none w-full  ${error.startsWith('Sorry')? 'border-red pr-8': 'border-[#DFDDE3]'}`} placeholder="Email address"
        ...
        <label htmlFor="email" className={`text-[9px]    ${error!=='Incorrect password'? 'text-red': 'text-grey'}`}>{error!=='Incorrect password'?error : 'We will be verifying this mail'}</label>
        </div>
        <div className="w-full  flex  flex-col gap-1">
        <div className="relative w-full flex items-center justify-center">
        <input className={`h-[40px] py-1 pl-3 pr-8  bg-lightGrey text-black  text-sm rounded-lg border border-[#DFDDE3] focus:ring-2  ring-purple outline-none   w-full line-clamp-1 ${error==='Incorrect password'? 'border-red pr-8': 'border-[#DFDDE3]'} `}
        ...
        <Link href={'/auth/forgot-password'} className="underline text-purple norm-mid  text-[17px] text-center md:text-sm">
        Forgot password?
        </Link>
        </div>
        </section>
