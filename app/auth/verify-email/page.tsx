...
import loadingPurlple from '~/public/images/load-purple.svg'
  ...
  const [isSucessful, setIsSucessful]=useState(false)
    ...
    return (
        ...
        <section className="h-full flex flex-1 items-center justify-center  bg-white">
            {isSucessful?(<div className='flex items-center flex-col gap-5'>
<Image src={loadingPurlple} className='w-12' alt=''/>
<h1 className='text-[43px] fancy text-black text-center  lg:text-4xl  md:text-3xl '>
Setting things up...
</h1>
            </div>): ( <div className='flex flex-col gap-4 w-[290px] items-center'>
              <div className=" items-center  hidden sm:flex mx-auto">
        <h1 className="text-3xl font-[family-name:var(--font-instrument)]  lg:text-2xl  ">Clam</h1>
        <Image src={wallet} alt=''  className="w-[26px]"/>
        </div>
 <h1 className="fancy text-[43px] text-center lg:text-3xl">Verify your mail</h1>
 <h1 className='text-[17px]   text-black  leading-6  text-center md:text-sm'>
    We{`'`}ve  sent you a mail at <span className='norm-mid'>{email}</span>. Enter the code in the field below
 </h1>
          ...
          <div className="flex justify-between gap-2 w-full text-white">
            {verificationCode.map((digit, index) => (
              <input
                ...
                maxLength={1}
                className={`text-[22px]  font-semibold outline-none px-2 py-3 rounded-md w-full bg-lightGrey text-center focus:ring-2 ring-purple  w-[48px]  h-[56px] fancy text-black grow-0 shrink-0 max-w-[48px]  max-h-[56px]  ${error && 'border border-red' }`}
                placeholder='.'
              />
            ))}
          </div>
...
<h1 className='text-sm text-red'>Code is incorrect</h1>
        ...
        <button  className={`bg-purple text-center text-sm  norm-mid  h-10 rounded-full text-white w-full    ${isVerificationCodeComplete()?'opacity-100 hover:ring ring-offset-2 hover:ring-[2px]  ring-purple duration-300':'opacity-40'}`} 
        disabled={isVerifying || !isVerificationCodeComplete()} onClick={handleVerify}>
            {isVerifying? <Image src={loadingGif} alt="" className="w-10 mx-auto"/>: 'Continue'}
        
        </button>

        <Link href={'/auth/sign-up'} className=" text-purple norm-mid  text-[17px] text-center  md:text-sm">
Cancel
        </Link>
 </div>)}
