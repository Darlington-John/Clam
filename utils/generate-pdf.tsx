...
export function generatePDF(bookData: any, user: any) {
   ...
   const details = [
      ...
      { header: 'BOOK NAME', body: bookData?.name },
      { header: 'INCOME', body: `$${bookData?.incomeTotal}` },
      { header: 'EXPENSE', body: `$${bookData?.expenseTotal}` },
   ];
   const sectionX = 10;
   const sectionY = 35;
   ...
   const columnWidth = availableWidth / numDetails;
   ...
   const formattedData = Array.isArray(bookData?.entries)
      ? bookData.entries.map((entry: any) => {
           return {
              amount: `$${entry?.amount}`,
              typeIcon: '',
   ...
   doc.autoTable({
      ...
      didDrawCell: function (data: any) {
         if (data.column.dataKey === 'typeIcon' && data.row.index >= 0) {
   ...
   doc.save(`${bookData?.name}_statement.pdf`);
